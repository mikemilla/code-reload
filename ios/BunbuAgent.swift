...
final class BunbuAgent {
    static let shared = BunbuAgent()

    private weak var viewModel: BunbuViewModel?
    private var store: BunbuFileStore?
    private let checkpoints = BunbuCheckpointStore.shared

    private var client: BunbuAiClient?
    ...
    func configure(viewModel: BunbuViewModel, store: BunbuFileStore) {
    ...
    func setConfig(apiKey: String, provider: String, modelId: String, maxTokens: Int) {
        client = BunbuAiClient(config: .init(
            apiKey: apiKey,
            provider: provider,
            modelId: modelId,
            maxTokens: maxTokens
    ...
    func send(_ text: String) {
        ...
        let system = BunbuSystemPrompt.withContext(files: files, targetPath: target)
    ...
    private func applyResponse(_ response: String, target: String) {
        guard let store = store,
              let code = BunbuSystemPrompt.extractCodeBlock(response),
