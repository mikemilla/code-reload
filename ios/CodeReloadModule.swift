...
@objc(CodeReloadModule)
class CodeReloadModule: RCTEventEmitter, CodeReloadHost, UIAdaptivePresentationControllerDelegate {
    static weak var shared: CodeReloadModule?

    private var fabWindow: CodeReloadFABWindow?
    private var sheetWindow: UIWindow?
    private weak var presentedSheet: UIViewController?
    private let viewModel = CodeReloadViewModel.shared
    ...
    override init() {
        super.init()
        CodeReloadModule.shared = self
        viewModel.host = self
        CodeReloadAgent.shared.configure(viewModel: viewModel, store: store)
    }
    ...
    func configureAgent(_ config: [String: Any]) {
        ...
        CodeReloadAgent.shared.setConfig(apiKey: apiKey, provider: provider, modelId: modelId, maxTokens: maxTokens)
    }
    ...
    func onRuntimeError(_ message: String) {
        DispatchQueue.main.async {
            ...
            CodeReloadAgent.shared.noteRuntimeError(message)
        }
    }
    ...
    @objc func initialize() {
        DispatchQueue.main.async {
            ...
            self.fabWindow = CodeReloadFABWindow(windowScene: scene) { [weak self] in
                self?.showSheet()
            }
        }
    }
    ...
    @objc func showSheet() {
        DispatchQueue.main.async {
            ...
            let sheetView = CodeReloadSheetView(viewModel: self.viewModel)
    ...
    func sendAgentMessage(_ text: String) {
        CodeReloadAgent.shared.send(text)
    }

    func stopAgent() {
        CodeReloadAgent.shared.stop()
    }
    ...
    func emit(_ type: String, payload: Any? = nil) {
        ...
        sendEvent(withName: "CodeReloadEvent", body: body)
    }
