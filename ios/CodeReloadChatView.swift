...
struct CodeReloadChatView: View {
    ...
    private var composer: some View {
        HStack(spacing: 8) {
            ...
            Button(action: viewModel.isStreaming ? { viewModel.stopGeneration() } : send) {
                Image(systemName: viewModel.isStreaming ? "stop.fill" : "arrow.up")
                    ...
                    .background(viewModel.isStreaming ? Color.red.opacity(0.8) : CodeReloadColors.accent)
                    .clipShape(Circle())
            }
        ...
        .padding(.horizontal, 12)
        .padding(.vertical, 8)
        .background(CodeReloadColors.surfaceElevated)
    }
    ...
    var body: some View {
        HStack {
            ...
            Text(message.text.isEmpty && !message.isUser ? "..." : message.text)
                .font(.system(size: 14))
                .foregroundStyle(message.text.isEmpty ? CodeReloadColors.textMuted : CodeReloadColors.textPrimary)
                .padding(.horizontal, 12)
                .padding(.vertical, 8)
                .background(message.isUser ? CodeReloadColors.accent : CodeReloadColors.surface)
                .cornerRadius(16)
                .lineLimit(nil)
