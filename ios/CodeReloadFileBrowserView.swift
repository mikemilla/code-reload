...
struct CodeReloadFileBrowserView: View {
    ...
    var body: some View {
        Group {
            if viewModel.openFilePath != nil {
                CodeReloadCodeEditorView(viewModel: viewModel)
            } else {
                fileList
            }
        }
                ...
                Button(action: { withAnimation(.easeInOut(duration: 0.12)) { expanded.toggle() } }) {
                    HStack(spacing: 8) {
                        ...
                        Image(systemName: "folder.fill")
                            .foregroundStyle(CodeReloadColors.accent)
                        Text(node.name)
...
struct CodeReloadCodeEditorView: View {
    @ObservedObject var viewModel: CodeReloadViewModel
