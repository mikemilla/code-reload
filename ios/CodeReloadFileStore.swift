...
///
/// Files live as real artifacts under `Documents/codereload/` so they can be pulled
...
final class CodeReloadFileStore {
    static let shared = CodeReloadFileStore()

    /// Directory names inside the project root that are not part of the
    /// editable source set (git metadata, checkpoint snapshots, codereload state).
    private let reservedDirs: Set<String> = [".git", ".checkpoints", ".codereload"]

    private let fm = FileManager.default
    private let queue = DispatchQueue(label: "dev.codereload.filestore", attributes: .concurrent)
    ...
    private init() {
        ...
        rootURL = docs.appendingPathComponent("codereload", isDirectory: true)
    ...
    func restore(_ files: [String: String]) {
        queue.sync(flags: .barrier) {
            // Remove existing editable files (keep .git/.checkpoints/.codereload).
            for rel in collectFiles() {
    ...
    // MARK: - CodeReload state (snapshot hash, etc.)

    private var stateURL: URL { rootURL.appendingPathComponent(".codereload/state.json") }
    ...
    /// Merge `state` into the persisted codereload state (does not clobber other keys).
