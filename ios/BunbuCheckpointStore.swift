...
struct BunbuCheckpoint: Identifiable {
    let id: String
    let timestamp: Date
    let label: String
}
...
/// `Documents/bunbu/.checkpoints/`. Fast, offline, and independent of git.
final class BunbuCheckpointStore {
    static let shared = BunbuCheckpointStore()
    ...
    func list() -> [BunbuCheckpoint] {
        ...
        for url in contents where url.pathExtension == "json" {
            ...
            result.append(BunbuCheckpoint(id: id, timestamp: Date(timeIntervalSince1970: ts), label: label))
        }
